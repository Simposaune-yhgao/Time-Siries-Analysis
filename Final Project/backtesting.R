...
library('timeSeries')
library('fPortfolio')
data1<-read.csv("daliguang.csv")

da1 = data1$日報酬率
tdx = c(1:978)
plot(tdx[1:978],da1,type ='l',xlab = 'data period',ylab = 'data1
     numerical')
...
tdxx = c(1:978)
plot(tdx[1:977],diffdata1,type ='l',xlab = 'data period',ylab = 'data1
     numerical')
...
summary(mr2)#AIC較低但omega變得更不顯著，t=7.208#mu&omega is not significant,whattodo?
...
mr3 = garchFit(~arma(1,1)+garch(1,1),data =diffdata1,trace = F,cond.dist = "std")
summary(mr3)
resi=residuals(mr3)
tsdiag(mr3,gof=12)

tangencyPortfolio(da1, spec = spec, constraints = "LongOnly")
